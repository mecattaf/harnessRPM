...
SPEC="fgp-browser.spec"
REPO="fast-gateway-protocol/browser"
...
if [ "$LATEST" != "$CURRENT" ]; then
    ...
    if curl --output /dev/null --silent --head --fail "$SOURCE_URL"; then
        ...
        git add "$SPEC"
        git commit -m "Update fgp-browser to $LATEST [build-rust]"
        git push
    ...
    echo "fgp-browser spec file is already at the latest version: $CURRENT"
fi
